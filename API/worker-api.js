...
export default {
       ...
       const db = env.Db;
        ...
        // Unified login/signup — returns token
            ...
            if (!username || !password) {
                ...
                const existing = await db.prepare('SELECT id FROM users WHERE LOWER(username) = LOWER(?)')
                    .bind(username).first();
                if (existing) {
                    response = new Response(JSON.stringify({
                        success: false,
          ...
          `).bind(username, passwordHash).first();
            ...
            `).bind(defaultMuted, result.id).run();
                ...
                const user = await db.prepare('SELECT id, username, password_hash FROM users WHERE LOWER(username) = LOWER(?)')
                    .bind(username).first();
  ...
  `;
            ...
            try {
                const mediaList = await db.prepare(query).bind(...binds).all();
                ...
                if (!user || !user.is_admin) {
                    ...
                    if (!files || !Array.isArray(files)) {
                        ...
                        for (const file of files) {
                            ...
                            const signUrl = new URL(`https://${env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${bucketName}/${objectKey}`);
          ...
          try {
                    ...
                    if (addedTags.length > 0 || removedTags.length > 0) {
                        for (const key of keys) {
                            const item = await db.prepare("SELECT tags FROM media WHERE object_key = ?")
                                .bind(key).first();
                            if (item) {
                                ...
                                await db.prepare("UPDATE media SET tags = ? WHERE object_key = ?")
                    ...
                    if (newDate) {
                        for (const key of keys) {
                            await db.prepare("UPDATE media SET created_date = ? WHERE object_key = ?")
            ...
            if (!user) {
                ...
                try {
                    ...
                    // Fetch stored hash
                    const stmt = db.prepare("SELECT password_hash FROM users WHERE id = ?").bind(user.id);
        ...
        // GET /admin-users — list all users (admin only)
            ...
            if (!user || user.username !== "lunepusa") {
                ...
                const users = await db.prepare("SELECT id, username, is_admin, subscription_expires, favorite_tags, muted_tags FROM users").all();
                response = Response.json({
                    users: users.results
                });
            }
        ...
        `).first();
            ...
            const current = await db.prepare("SELECT favorite_tags, muted_tags FROM users WHERE id = ?")
                .bind(user.id).first();
                    ...
                    if (!share) {
                        ...
                        if (user && share.target_type === 'date') {
                            ...
                            if (!dates.includes(share.target_value)) {
                                ...
                                await db.prepare("UPDATE users SET purchased_dates = ? WHERE id = ?")
                ...
                try {
                    ...
                    if (!platform || !username) {
                        ...
                        if (exists) {
                            ...
                            pairs.push(newPair);
                            await db.prepare('UPDATE users SET saved_payment_pairs = ? WHERE id = ?')
            ...
            if (!user) {
                ...
                try {
                    ...
                    if (!platform || !username) {
                        ...
                        const platRow = await db.prepare("SELECT senders FROM platforms WHERE platform = ?")
                            .bind(platform).first();
                        ...
                        if (!listData.messages || listData.messages.length === 0) {
                            ...
                            for (const msg of listData.messages) {
                                ...
                                await db.prepare('UPDATE users SET wallet = ? WHERE id = ?')
    ...
    try {
        ...
        const { results } = await db.prepare(
            "SELECT * FROM media WHERE tags LIKE '%delete%'"
        ).all();
        ...
        for (const row of results) {
            ...
            if (tags.includes("delete")) {
                ...
                if (row.file_type && row.file_type.startsWith("video/")) {
                    const thumbKey = row.object_key.replace(/\.[^/.]+$/, "") + "_thumb.jpg";
                    await bucket.delete(thumbKey);
                }

                // 5. Delete the row from D1
                await db.prepare("DELETE FROM media WHERE object_key = ?")
                    .bind(row.object_key)
                    .run();
