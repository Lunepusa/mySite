...
if (!token) {
    return Response.json({ code: "BAD_INPUT", error: "Invalid share link" }, { status: 400 });
  }

  const share = await db
    .prepare("SELECT target_type, target_value FROM share_links WHERE token = ?")
    .bind(token)
    .first();
    
  if (!share) {
    return Response.json({ code: "NOT_FOUND", error: "Invalid share link" }, { status: 404 });
  }
