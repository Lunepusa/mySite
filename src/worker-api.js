...
const bucket = env.BUCKET_NAME || 'lunepusa';
        ...
        // Unified login/signup — returns token
          ...
          `).bind(username, passwordHash).first();
                    if (result) {
                        ...
                        const defaultMuted = 'taboo,scat,fart,cnc,dubcon,race_play,bnwo,age play,blackmail fetish,blood,breathplay,hypno,somno,monster_dildo,knotted_dildo,tentacle,werewolf';
                        await env.Db.prepare(`
              UPDATE users SET muted_tags = ? WHERE id = ?
            `).bind(defaultMuted, result.id).run();
                        const payload = {
                            userId: result.id,
                            username,
                ...
                const user = await env.Db.prepare('SELECT id, username, password_hash FROM users WHERE LOWER(username) = LOWER(?)')
                    .bind(username).first();
        ...
        // Admin upload-batch (legacy)
            ...
            if (!user || !user.is_admin) {
                ...
                VALUES (?, ?, '', 'lunepusa', 0, ?)
              `).bind(objectKey, createdDateInt, file.type).run();
                            successCount++;
                        } catch (e) {
                            errors.push({
                                file: file.name,
            ...
            // ────────────────────────────────────────────────
            if (q.trim()) {
                const deurl = q.replaceAll("%7E", "~").replaceAll("%2B", "+").replaceAll("%2D", "-");
  ...
  `;
            ...
            try {
                const mediaList = await env.Db.prepare(query).bind(...binds).all();
        ...
        // ====================== PRESIGN ======================
            ...
            try {
                const user = await getUser(request, env);
                if (!user || !user.is_admin) {
                    response = withCors(new Response('Unauthorized', {
                        status: 401
                    }), request);
                } else {
                    let files;
                    try {
                        ...
                        response = withCors(Response.json({
                            error: "Invalid JSON"
                        }, {
                            status: 400
                        }), request);
                    ...
                    if (!files || !Array.isArray(files)) {
                       ...
                       const bucket = env.BUCKET_NAME || 'lunepusa';;
                        ...
                        for (const file of files) {
                            ...
                            if (cleanName.startsWith('PXL_')) cleanName = cleanName.substring(4);

                            const objectKey = `media/${cleanName}`;
                            const signUrl = new URL(`https://${env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${bucket}/${objectKey}`);
        ...
        `).bind(objectKey, createdDate, 'lunepusa_f,video', fileType).run();
        ...
        // POST /update-date — update file date/time, update D1
        else if (url.pathname === '/update-date' && request.method === 'POST') {
            const user = await getUser(request, env);
            if (!user || !user.is_admin) {
                response = new Response('Unauthorized', {
                    status: 401
                ...
                try {
                    ...
                    if (!oldKey || !newDate) {
                        throw new Error('Missing oldKey, newDate');
                    }
                    // Update D1
                    const newCreatedDate = parseInt(newDate, 10);
                    await env.Db.prepare(`
            UPDATE media
            SET created_date = ?
            WHERE object_key = ?
          `).bind(newCreatedDate, oldKey).run();
                    response = Response.json({
                        success: true,
                        newKey
                    });
            ...
            try {
                ...
                response = new Response(`Access token success: ${token.slice(0,10)}...`);
            } catch (err) {
                ...
                response = new Response('Unauthorized', {
                    ...
                    if (addedTags.length > 0 || removedTags.length > 0) {
                        for (const key of keys) {
                            const item = await env.Db.prepare("SELECT tags FROM media WHERE object_key = ?")
                                .bind(key).first();
                            if (item) {
                                let current = (item.tags || "").split(",").map(t => t.trim()).filter(t => t);
                                // Remove first
                                ...
                                await env.Db.prepare("UPDATE media SET tags = ? WHERE object_key = ?")
                                    .bind(updated, key).run();
                            }
                        }
                    }
                    ...
                    if (newDate) {
                        for (const key of keys) {
                            await env.Db.prepare("UPDATE media SET created_date = ? WHERE object_key = ?")
                                .bind(parseInt(newDate, 10), key).run();
                        }
                    }
                }
            ...
            if (!user) {
                ...
                try {
                    ...
                    // Fetch stored hash
                    const stmt = env.Db.prepare("SELECT password_hash FROM users WHERE id = ?").bind(user.id);
        ...
        // GET /admin-users — list all users (admin only)
            ...
            if (!user || user.username !== "lunepusa") {
                ...
                const users = await env.Db.prepare("SELECT id, username, is_admin, subscription_expires, favorite_tags, muted_tags FROM users").all();
                response = Response.json({
                    users: users.results
                });
            }
        ...
        // POST /admin-update-user — update admin status or expiry
            ...
            try {
                const all = await env.Db.prepare("SELECT tags FROM media").all();
                const countMap = {};
        ...
        `).first();
            ...
            const current = await env.Db.prepare("SELECT favorite_tags, muted_tags FROM users WHERE id = ?")
                .bind(user.id).first();
            const newFavorite = updates.favorite_tags !== undefined ? updates.favorite_tags : (current.favorite_tags || '');
            const newMuted = updates.muted_tags !== undefined ? updates.muted_tags : (current.muted_tags || '');
            await env.Db.prepare(`
        UPDATE users SET favorite_tags = ?, muted_tags = ? WHERE id = ?
      `).bind(newFavorite, newMuted, user.id).run();
            response = Response.json({
                success: true
            });
        }
            ...
            if (!user || !user.is_admin) {
                ...
                try {
                    ...
                    if (!['date', 'media'].includes(target_type) || !target_value) {
                        ...
                        ).bind(token, target_type, target_value, user.id).run();
                ...
                try {
                    ...
                    if (!share) {
                        ...
                        if (user && share.target_type === 'date') {
                            ...
                            const dates = current.split(',').map(d => d.trim()).filter(Boolean);
                            if (!dates.includes(share.target_value)) {
                                const updated = current ? `${current},${share.target_value}` : share.target_value;
                                await env.Db.prepare("UPDATE users SET purchased_dates = ? WHERE id = ?")
                                    .bind(updated, user.id).run();
                            }
                        }
                        // Fetch only shared content
                        let query = "SELECT object_key, created_date, caption, tags, file_type FROM media WHERE 1=1";
                        let binds = [];
        ...
        // GET /saved-payment-pairs — return the user's current saved pairs
        else if (url.pathname === '/saved-payment-pairs' && request.method === 'GET') {
            const user = await getUser(request, env);
            if (!user) {
                response = new Response('Unauthorized', {
                    status: 401
                ...
                try {
                    let pairs = [];
                    if (user.saved_payment_pairs && user.saved_payment_pairs.trim() !== '[]') {
                        try {
            ...
            if (!user) {
                ...
                try {
                    ...
                    if (!platform || !username) {
                        ...
                        if (rawPairs && rawPairs.trim() !== '' && rawPairs.trim() !== 'null' && rawPairs.trim() !== '[]') {
                            try {
                        ...
                        const exists = pairs.some(p =>
                            p.platform === newPair.platform && p.username === newPair.username
                        );
                        ...
                        if (exists) {
                            ...
                            pairs.push(newPair);
                            await env.Db.prepare('UPDATE users SET saved_payment_pairs = ? WHERE id = ?')
            ...
            if (!user) {
                ...
                try {
                    ...
                    if (!platform || !username) {
                        ...
                        const platRow = await env.Db.prepare("SELECT senders FROM platforms WHERE platform = ?")
                            .bind(platform).first();
                        ...
                        if (!listData.messages || listData.messages.length === 0) {
                            ...
                            for (const msg of listData.messages) {
                                ...
                                if (labels.includes('Label_1909551957303359400') || labels.includes('Label_2723636466645469159')) {
                                    continue;
                                }
                                ...
                                await env.Db.prepare('UPDATE users SET wallet = ? WHERE id = ?')
            ...
            if (!user) {
                ...
                try {
                    ...
                    for (const row of results) {
                        ...
                        const conflictCheck = await db.prepare("SELECT id, tags FROM media WHERE object_key = ? AND id != ?")
                            .bind(newKey, row.id)
                            .first();
                        ...
                        if (conflictCheck) {
                            if (row.id < conflictCheck.id) {
                                ...
                                await db.prepare("UPDATE media SET object_key = ?, tags = ? WHERE id = ?")
            ...
            // <--- HERE: Close the 'else' block for migrate-flatten-r2
